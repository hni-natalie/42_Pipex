...
/*   Updated: 2024/11/26 14:28:34 by hni-xuan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*  
Concept:
1. Go through the string and count how many word can be formed
2. 
*/
...
static char	*skip_delimiter(char const *s, char c)
{
	while (*s && *s == c)
		s++;
	return ((char *)s);
...
/* int main(void)
{
	char str[] = "   Are you okayy yess no    ha ha  ha  ";
	char s = ' ';
	char **ptr = ft_split(str, s);
	printf("%d\n", word_count(str, s));
	for (int i = 0; i < word_count(str, s); i++)
		printf("%s\n", ptr[i]);
	free(ptr);
} */
